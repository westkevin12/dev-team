...
app = FastAPI(
    title="dev-team",
    description="API for interacting with the Agent dev-team",
)
...
def stream_messages(
    ...
    """
    processed_config: RunnableConfig = ensure_valid_config(config=config)

    # LangGraph's default recursion_limit is 25.
    # Increase it to allow for more complex agent interactions.
    # If a recursion_limit is already set in the incoming config,
    # ensure it's at least 50, otherwise set it to 50.
    if processed_config.get("recursion_limit", 0) < 50:
        processed_config["recursion_limit"] = 50
    set_tracing_properties(processed_config)
    input_dict = input.model_dump()

    for data in agent.stream(input_dict, config=processed_config, stream_mode="messages"):
        yield dumps(data) + "\n"
